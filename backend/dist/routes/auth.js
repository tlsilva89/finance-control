...
router.post("/register", async (req, res) => {
    try {
        ...
        const { username, name, birthDate, password, securityQuestion, securityAnswer, } = req.body;
...
router.post("/check-username", async (req, res) => {
    try {
        ...
        if (!usernameRegex.test(username)) {
            return res.json({
                available: false,
                message: "Nome de usuário deve ter entre 3-20 caracteres e conter apenas letras, números e underscore",
            });
        }
        const existingUser = await prisma.user.findUnique({
            where: { username },
        ...
        console.log(`${available ? "✅" : "❌"} Username ${username} ${available ? "disponível" : "já em uso"}`);
        res.json({
            available,
            message: available
...
router.post("/forgot-password", async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            return res.json({
                message: "Se o usuário existir, você poderá prosseguir com a recuperação",
                hasUser: false,
            });
        }
...
router.post("/reset-password", async (req, res) => {
    try {
        ...
        const isValidAnswer = await bcrypt.compare(securityAnswer.toLowerCase().trim(), user.securityAnswer);
        if (!isValidAnswer) {
...
router.post("/change-password", authenticateToken, async (req, res) => {
    try {
        ...
        if (!user) {
            console.log("❌ Usuário não encontrado para alteração de senha:", userId);
...
router.post("/update-security-question", authenticateToken, async (req, res) => {
    try {
